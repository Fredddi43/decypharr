...
<!-- Usenet routing settings for this debrid live under
    ...
    addArrConfig(data = {}) {
        ...
        // Re-render the Usenet panel — it lists per-arr credentials, so
        // an arr add/edit changes the visible rows.
        if (this.refs && this.refs.debridConfigs) {
            this.renderUsenetDebridPanel();
        }
    }
            ...
            <div class="card bg-base-100 border border-base-300 shadow-sm arr-config ${isAutoDetected ? 'border-info' : ''}" data-index="${index}">
                <div class="card-body">
                    <div class="flex justify-between items-start mb-4">
                        ...
                        ${!isAutoDetected ? `
                            <button type="button" class="btn btn-error btn-sm" onclick="this.closest('.arr-config').remove(); window.configManager && window.configManager.renderUsenetDebridPanel();">
                                <i class="bi bi-trash"></i>
                            </button>
                        ` : ''}
                    </div>
