...
Every pass also walks **Decypharr's own queue** for two cases:

**`state=error`** (magnets every configured debrid rejected — DMCA / 451,
quota exhausted with no fallback, etc.). For each it:
